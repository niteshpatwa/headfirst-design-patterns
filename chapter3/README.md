# DECORATOR DESIGN PATTERN
- Attach additional responsibilities to an object dynamically.
- Decorator provide a flexible alternative to subclassing for extending functionality.

## Project 1: IODecorator
- Real World Example Project
- Decorates input streams to add functionality like case conversion.
![alt text](../resources/3.1.png)

## Project 2: Starbuzz
- Decorator Design Pattern Example
- Beverage orders with condiments as decorators wrapping base drinks to calculate costs dynamically.
![alt text](../resources/3.2.png)
